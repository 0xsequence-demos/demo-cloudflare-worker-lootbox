...
import { formatStatString, isLessThan24Hours, wait } from './utils'
...
export async function handleRequest(request: any, env: Env, ctx: ExecutionContext) {
	...
	async function uploadAsset(projectID: any, collectionID: any, assetID: any, tokenID: any, url: any) {
	...
	async function upload(name: any, attributes: any, imageUrl: any) {
		let METADATA_URL;
		...
		try {
			...
			// upload asset
			const uploadAssetRes = await uploadAsset(projectID, collectionID, res2.asset.id, String(randomTokenIDSpace), imageUrl)
	...
	async function getInferenceWithItem(prompt: string) {
		try {
			const response = await fetch(`https://api.cloud.scenario.com/v1/models/${env.SCENARIO_MODEL_ID}/inferences`, {
				method: 'POST',
				headers: {
	...
	async function getInferenceStatus(id: any) {
		...
		while (!['succeeded', 'failed'].includes(status)) {
			...
			await wait(5000) // Polling every 5 seconds
		}
	...
	async function fullPaginationDay(address: string) {
		const txs: any = []
	...
	async function hasDailyMintAllowance(address: string) {
		const txs = await fullPaginationDay(address)
		const count = mintCount(txs)
		return count < env.DAILY_MINT_RESTRICTION
	}


	async function callContract(collectibleAddress: string, address: string, tokenID: number): ethers.providers.TransactionResponse {
	...
	if (referer.toString() == env.CLIENT_URL) {
		...
		try {
			...
			if (mint) {
				try {
					const txn = await callContract(env.CONTRACT_ADDRESS, address, tokenID)
				...
				const id = await getInferenceWithItem(loot.loot.name)
				const inferenceObject = await getInferenceStatus(id)
				const response = await upload(loot.loot.name + " " + loot.loot.type, loot.attributes, inferenceObject.inference.images[0].url)
